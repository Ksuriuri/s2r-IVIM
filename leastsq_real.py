...
if __name__ == "__main__":
    ...
    process_num = 32  # must small than cpu num
    # define b values
    ...
    save_path = r'/home/public/Documents/hhy/IVIM/UDA_real/leastsq'
    ...
    for idx, bf in enumerate(b_files):
        ...
        X_test = np.expand_dims(test_data['x'][17], 0)
        x_fit = X_test[..., 1:]
        print(x_fit.shape)
        ...
        time_per_pixel = time_per_iter / (1 * X_test.shape[1] * X_test.shape[2])
        print('time per pixel ', time_per_pixel)
        x_fit = np.reshape(x_fit, (X_test.shape[0], X_test.shape[1], X_test.shape[2], 8))
        ivim_pre = np.reshape(ivim_pre, (x_fit.shape[1], x_fit.shape[2], 3))
        # for i in range(3):
        ...
        # plt.subplot(131)
        # plt.imshow(ivim_pre[..., 0], cmap='gray')  # , clim=(0, 1)
        # plt.colorbar()
        # plt.axis('off')
        # plt.subplot(132)
        # plt.imshow(ivim_pre[..., 1], cmap='gray')  # , clim=(0, 0.002)
        # plt.colorbar()
        # plt.axis('off')
        # plt.subplot(133)
        # plt.imshow(ivim_pre[..., 2], cmap='gray')  # , clim=(0, 1)
        # plt.colorbar()
        # plt.axis('off')
        # plt.show()

        np.savez(join(join(save_path, save_name)), ivim=ivim_pre, x_fit_pre=x_fit_pre, x_fit=x_fit, time=time_per_pixel)
