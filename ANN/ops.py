...
def upsample(input_data, shortcut_size, name, method="deconv"):
    ...
    if method == "deconv":
        ...
        output = tf.layers.conv2d_transpose(input_data, num_filter//2, kernel_size=3, padding='same',
...
def batchnorm(net, decay=0.9, epsilon=0.00001, act=tf.identity, is_train=False, beta_init=tf.zeros_initializer,
        gamma_init=tf.random_normal_initializer(mean=1.0, stddev=0.002), name='batchnorm_layer'):
    ...
    with tf.variable_scope(name) as vs:
        ...
        if is_train:
            ...
            outputs = act( tf.nn.batch_normalization(net, mean, var, beta, gamma, epsilon) )
        else:
            outputs = act( tf.nn.batch_normalization(net, moving_mean, moving_variance, beta, gamma, epsilon) )
